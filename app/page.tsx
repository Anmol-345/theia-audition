        ...
        <div className="relative z-10 h-full w-full flex flex-col justify-between">
          ...
          <header className="flex items-start justify-between w-full" data-purpose="main-header">
            ...
            <div className="flex flex-col space-y-0.5">
              <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight text-white cinematic-glow leading-none">
                Theia
              </h1>
              <p className="text-[10px] sm:text-sm md:text-base font-normal tracking-wide text-neutral-300/90 pl-0.5">
                Photography and Film society
              </p>
            </div>

            {/* Nav — icon-only on mobile, icon + label on sm+ */}
            <nav aria-label="Main Navigation" className="flex items-center gap-2 sm:gap-3 md:gap-4">
              {/* LinkedIn */}
              <a
                aria-label="Theia LinkedIn"
                className="social-pill !px-2.5 sm:!px-6"
                href="https://www.linkedin.com/company/theia-the-photography-society-of-srmuh/"
                target="_blank"
              ...
              >
                ...
                <span className="relative z-10 hidden sm:inline">Theia</span>
              </a>
              {/* Instagram */}
              <a
                aria-label="Instagram thetheiamedia"
                className="social-pill !px-2.5 sm:!px-6"
                href="https://www.instagram.com/thetheiamedia/"
                target="_blank"
                rel="noopener noreferrer"
              >
                <svg className="w-4 h-4 sm:w-5 sm:h-5 shrink-0 relative z-10" fill="currentColor" viewBox="0 0 24 24">
                ...
                <span className="relative z-10 hidden sm:inline">thetheiamedia</span>
              </a>
            </nav>
          </header>
          ...
          <div className="flex md:hidden flex-col items-center text-center flex-1 justify-center gap-3 -mt-8 pb-[30vh]">
            ...
            >
              <div className="text-4xl font-extrabold tracking-tight text-white cinematic-glow">
                Auditions 2k26
              </div>
              <div className="inline-flex items-center gap-2 mt-2 text-xs text-neutral-400 group-hover:text-white transition-colors duration-300 tracking-wider uppercase font-medium">
          ...
          {/* ── BOTTOM ROW ─────────────────────────────────────── */}
          <footer className="flex flex-col md:flex-row items-end justify-between w-full gap-4 md:gap-4 pb-2">

            {/* Interest Selector */}
            <div className="flex flex-col items-start space-y-2 sm:space-y-3 self-start md:self-end w-full md:w-auto">
              <h2 className="text-sm sm:text-lg md:text-xl font-normal italic tracking-wide text-neutral-300/95">
                Whats your interest?
              </h2>
              <div aria-label="Interest Categories" className="flex flex-wrap items-center gap-2.5 sm:gap-4 relative" role="group">
                {INTERESTS.map((interest) => (
                  <div key={interest.id} className="relative group flex flex-col items-center">
                    <button
                      aria-label={interest.name}
                      className={`glass-circle relative w-10 h-10 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-white/50 ${activePill === interest.id ? "ring-2 ring-white bg-white/40" : ""}`}
                      type="button"
                      onClick={() => handleInterestClick(interest.id, interest.name)}
                    >
                      <span className={interest.animClass}>{interest.icon}</span>
                    </button>
                    <span className="absolute -top-9 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/80 backdrop-blur-md text-white text-xs px-2.5 py-1 rounded-full whitespace-nowrap pointer-events-none border border-white/20">
                      {interest.name}
                    </span>
                  </div>
                ))}
